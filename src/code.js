...
(() => {
  ...
  // src/app/card.ts
  var VIDEO_QR_SIZE = 20;
  var PRODUCT_IMAGE_FRAME_SIZE = 90;
  ...
  async function fillCardStatic(card, group) {
    ...
    const imgNode = findBestImageNode(card);
    if (imgNode && group.imageBytes) {
      resizeProductImageFrame(imgNode, group.imageWidth, group.imageHeight);
      ...
      if ("fills" in imgNode) {
        ...
        postLog(`${CONFIG.IMAGE} \u043D\u0430\u0439\u0434\u0435\u043D, \u043D\u043E \u0443 \u0441\u043B\u043E\u044F \u043D\u0435\u0442 fills. \u0422\u0438\u043F \u0441\u043B\u043E\u044F: ${imgNode.type}`);
      }
    } else if (!imgNode && group.imageBytes) {
      postLog(`\u041D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D \u0441\u043B\u043E\u0439 ${CONFIG.IMAGE} \u0432 \u043A\u0430\u0440\u0442\u043E\u0447\u043A\u0435 ${getGroupDebugName(group)}`);
    }
  ...
  function findBestImageNode(card) {
    const nodes = card.findAll((n) => n.name === CONFIG.IMAGE);
    if (nodes.length === 0) return null;
    const fillNodes = nodes.filter((node) => "fills" in node);
    const candidates = fillNodes.length ? fillNodes : nodes;
    return [...candidates].sort((a, b) => scoreImageNode(b) - scoreImageNode(a))[0] || null;
  }
  function scoreImageNode(node) {
    let score = 0;
    if ("fills" in node) score += 1e3;
    if (node.type === "RECTANGLE") score += 80;
    if (node.type === "FRAME") score += 60;
    const w = Number(node.width) || 0;
    const h = Number(node.height) || 0;
    const area = w * h;
    if (area > 0) score += Math.min(250, area / 20);
    score -= Math.abs(w - PRODUCT_IMAGE_FRAME_SIZE) * 2;
    score -= Math.abs(h - PRODUCT_IMAGE_FRAME_SIZE) * 2;
    return score;
  }
  function getGroupDebugName(group) {
    var _a, _b;
    return group.headerName || group.mainSku || ((_b = (_a = group.items) == null ? void 0 : _a[0]) == null ? void 0 : _b.sku) || "\u0431\u0435\u0437 \u043D\u0430\u0437\u0432\u0430\u043D\u0438\u044F";
  }
  function resizeProductImageFrame(node, imageWidth, imageHeight) {
    var _a, _b;
    const resizable = node;
    try {
      if ("layoutSizingHorizontal" in resizable) resizable.layoutSizingHorizontal = "FIXED";
      if ("layoutSizingVertical" in resizable) resizable.layoutSizingVertical = "FIXED";
      if ("constraints" in resizable) {
        resizable.constraints = { horizontal: "MIN", vertical: "MIN" };
      }
    } catch (e) {
    }
    try {
      const size = fitProductImageSize(imageWidth, imageHeight);
      const oldX = Number(node.x) || 0;
      const oldY = Number(node.y) || 0;
      const oldW = Number(node.width) || PRODUCT_IMAGE_FRAME_SIZE;
      const oldH = Number(node.height) || PRODUCT_IMAGE_FRAME_SIZE;
      if (resizable.resizeWithoutConstraints) {
        resizable.resizeWithoutConstraints(size.width, size.height);
      } else {
        (_a = resizable.resize) == null ? void 0 : _a.call(resizable, size.width, size.height);
      }
      safeSetPosition(
        node,
        Math.round(oldX + (oldW - size.width) / 2),
        Math.round(oldY + (oldH - size.height) / 2),
        "product image"
      );
    } catch (e) {
      try {
        const size = fitProductImageSize(imageWidth, imageHeight);
        (_b = resizable.resize) == null ? void 0 : _b.call(resizable, size.width, size.height);
      } catch (e2) {
      }
    }
  }
  function fitProductImageSize(imageWidth, imageHeight) {
    const w = Number(imageWidth) || 0;
    const h = Number(imageHeight) || 0;
    if (w <= 0 || h <= 0) return { width: PRODUCT_IMAGE_FRAME_SIZE, height: PRODUCT_IMAGE_FRAME_SIZE };
    const scale = Math.min(PRODUCT_IMAGE_FRAME_SIZE / w, PRODUCT_IMAGE_FRAME_SIZE / h);
    return {
  ...
  async function appendAdPlaceholderIfRoom(column, pageNum, getRemainingHeight2) {
...
${safeWidth} x ${safeHeight}`;
  ...
  function buildLogText(report) {
    ...
    for (const run of report.runs || []) {
      lines.push(
        `- ${run.title || "\u041B\u0438\u0441\u0442\u043E\u0432\u043A\u0430"}: ${run.pages || 0} \u0441\u0442\u0440., ${run.products || 0} \u0442\u043E\u0432\u0430\u0440\u043E\u0432, \u0440\u0435\u043A\u043B\u0430\u043C\u043D\u044B\u0445 \u043C\u0435\u0441\u0442 ${run.adPlaceholders || 0}, \u043E\u0448\u0438\u0431\u043E\u043A \u043A\u0430\u0440\u0442\u043E\u0447\u0435\u043A ${run.cardErrors || 0}`
      );
      if (Array.isArray(run.adSizes) && run.adSizes.length > 0) {
        lines.push(`  \u0420\u0435\u043A\u043B\u0430\u043C\u043D\u044B\u0435 \u043C\u0435\u0441\u0442\u0430: ${run.adSizes.join("; ")}`);
      }
    }
    const cropReports = ((_a = report.uiStats) == null ? void 0 : _a.imageCropReports) || [];
    if (cropReports.length > 0) {
      lines.push("", "\u041E\u0431\u0440\u0435\u0437\u043A\u0430 \u0444\u043E\u0442\u043E:");
      for (const item of cropReports.slice(-20)) {
        lines.push(
          `- ${item.sku || "-"}: ${item.source || "?"} -> \u043E\u0431\u044A\u0435\u043A\u0442 ${item.content || "?"} -> crop ${item.crop || "?"} -> export ${item.export || "?"}`
        );
      }
    } else {
      lines.push("", "\u041E\u0431\u0440\u0435\u0437\u043A\u0430 \u0444\u043E\u0442\u043E: \u043D\u0435\u0442 \u0434\u0430\u043D\u043D\u044B\u0445");
    }
    return lines.join("\n");
  }
  ...
  async function layoutBrochureRun(run, runIndex, layout, masters, splitMetrics, state, opts, onProgress) {
    ...
    for (const group of orderedGroups) {
      ...
      for (let i = 0; i < cardsToPlace.length; i++) {
        ...
        preparedCards.push({ node: card });
      }
      await onProgress(1);
    }
    let cardIndex = 0;
    while (cardIndex < preparedCards.length) {
      ensureNotCancelled(opts);
      const pack = packNextSequentialPage(preparedCards, cardIndex, currentLayout.columns[0].height);
      placePackedCards(currentLayout.columns, preparedCards, cardIndex, pack);
      cardIndex = pack.endIndex;
      const hasMoreCards = cardIndex < preparedCards.length;
      if (hasMoreCards) {
        const pageAdSizes = await appendAdsForPackedPage(currentLayout.columns, state.pageNum);
        adPlaceholders += pageAdSizes.length;
        adSizes.push(...pageAdSizes);
    ...
    const footerAdSizes = await finalizeRunFooter(
      state,
      run.title,
      runIndex,
      localPageNum,
    ...
    if (footerAdSizes.length > 0) {
      adPlaceholders += footerAdSizes.length;
      adSizes.push(...footerAdSizes);
    }
    return {
  ...
  function packNextSequentialPage(cards, startIndex, columnHeight) {
    let best = null;
    for (let endIndex = startIndex + 1; endIndex <= cards.length; endIndex++) {
      const splitIndex = findBestSequentialSplit(cards, startIndex, endIndex, columnHeight);
      if (splitIndex === null) break;
      best = { endIndex, splitIndex };
    }
    if (best) return best;
    return {
      endIndex: Math.min(cards.length, startIndex + 1),
      splitIndex: startIndex + 1
    };
  }
  function findBestSequentialSplit(cards, startIndex, endIndex, columnHeight) {
    let bestSplit = null;
    let bestScore = Number.POSITIVE_INFINITY;
    for (let splitIndex = startIndex + 1; splitIndex <= endIndex; splitIndex++) {
      if (splitIndex === endIndex && endIndex - startIndex > 1) continue;
      const leftH = estimatePreparedStackHeight(cards, startIndex, splitIndex);
      const rightH = estimatePreparedStackHeight(cards, splitIndex, endIndex);
      if (leftH > columnHeight + FIT_HEIGHT_TOLERANCE) continue;
      if (rightH > columnHeight + FIT_HEIGHT_TOLERANCE) continue;
      const diff = Math.abs(leftH - rightH);
      const wasted = columnHeight - leftH + (columnHeight - rightH);
      const score = diff * 4 + wasted;
      if (score < bestScore) {
        bestScore = score;
  ...
  function estimatePreparedStackHeight(cards, startIndex, endIndex) {
    if (endIndex <= startIndex) return 0;
    let height = 0;
    for (let i = startIndex; i < endIndex; i++) {
      height += cards[i].node.height;
    }
    height += Math.max(0, endIndex - startIndex - 1) * CONFIG.ITEM_GAP;
    return height;
  }
  function placePackedCards(columns, cards, startIndex, pack) {
    const [left, right] = columns;
    for (let i = startIndex; i < pack.splitIndex; i++) {
      left.appendChild(cards[i].node);
    }
    for (let i = pack.splitIndex; i < pack.endIndex; i++) {
      right.appendChild(cards[i].node);
    }
  }
  async function appendAdsForPackedPage(columns, pageNum) {
    const sizes = [];
    for (const column of columns) {
      const size = await safeAppendAdPlaceholderIfRoom(
        column,
        pageNum,
        getRemainingHeight
      );
  ...
  async function safeAppendAdPlaceholderIfRoom(column, pageNum, getRemainingHeight2) {
    try {
      return await appendAdPlaceholderIfRoom(column, pageNum, getRemainingHeight2);
    } catch (err) {
  ...
  async function finalizeRunFooter(state, title, runIndex, localPageNum, boxNoticeBottomMaster = null, addPaginators = true) {
    ...
    const adSizes = [];
    const sparseAdSize = await safeBalanceSparseLastPageAndAddAd(state.lastPage, state.pageNum, reservedBottom);
    if (sparseAdSize) {
      adSizes.push(sparseAdSize);
    } else {
      const columns = getPageColumns2(state.lastPage);
      adSizes.push(...await appendAdsForPackedPage(columns, state.pageNum));
    }
